...
*Setup instructions for a Gitian build of Oscillate using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Oscillate CLI
...
```bash
cp oscillate/contrib/gitian/gitian-build.py .
```
...
```
git clone git@github.com:oscillate/gitian.sigs.git
...
gpg --output $VERSION-linux/$NAME/oscillate-linux-$VERSION-build.assert.sig --detach-sign $VERSION-linux/$NAME/oscillate-linux-$VERSION-build.assert
gpg --output $VERSION-osx-unsigned/$NAME/oscillate-osx-$VERSION-build.assert.sig --detach-sign $VERSION-osx-unsigned/$NAME/oscillate-osx-$VERSION-build.assert
gpg --output $VERSION-win-unsigned/$NAME/oscillate-win-$VERSION-build.assert.sig --detach-sign $VERSION-win-unsigned/$NAME/oscillate-win-$VERSION-build.assert
```

Make a pull request (both the `.assert` and `.assert.sig` files) to the
[oscillate/gitian.sigs](https://github.com/oscillate/gitian.sigs/) repository:
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/oscillate-linux-*-build.assert
    gpg --detach-sign ${VERSION}-win-unsigned/${SIGNER}/oscillate-win-*-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/oscillate-osx-*-build.assert
```
